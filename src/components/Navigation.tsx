...
export default function Navigation({ currentView, onViewChange }: NavigationProps) {
  ...
  return (
    <>
      {/* 移动端底部导航（< lg） */}
      <nav className="fixed bottom-0 left-0 right-0 z-20 border-t border-primary/10 bg-white/90 backdrop-blur-xl px-4 pb-6 pt-3 lg:hidden">
      ...
      {/* 桌面端左侧边栏（≥ lg） */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-60 flex-col bg-white border-r border-primary/10 z-20">
        {/* Logo 区域 */}
        <div className="flex items-center gap-3 px-6 py-6 border-b border-primary/10">
          ...
          <div>
            <p className="text-slate-900 font-extrabold text-sm leading-tight">成就丛林</p>
            <p className="text-slate-400 text-[10px]">Achievement Jungle</p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-primary/10">
          <p className="text-slate-400 text-[10px]">© 2025 成就丛林</p>
        </div>
      </aside>
    </>
  );
