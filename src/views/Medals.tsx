  ...
  return (
    ...
    >
      ...
      <nav className="px-6 mb-6">
        <div className="flex bg-slate-200/50 dark:bg-[var(--bg-card)] p-1 rounded-xl">
          {(['all', 'unlocked', 'locked'] as const).map(f => (
            <button
              key={f}
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${filter === f ? 'bg-primary text-slate-900 shadow-sm' : 'text-slate-600 dark:text-[var(--text-secondary)]'}`}
              onClick={() => setFilter(f)}
            >
        ...
        <section className="px-6 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-y-8 gap-x-4 py-4">
          {filteredMedals.map((medal) => (
            ...
            >
              ...
              <p className="text-[11px] font-extrabold text-slate-800 dark:text-[var(--text-primary)] text-center leading-tight tracking-wider">{medal.name}</p>
              {medal.unlocked && medal.unlocked_at && (
                <p className="text-[9px] text-slate-400 dark:text-[var(--text-muted)]">
        ...
        {selectedMedal && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-[var(--bg-surface)] rounded-3xl p-6 max-w-sm w-full shadow-2xl transition-colors"
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <div className={`w-full p-3 rounded-xl mb-4 ${selectedMedal.unlocked ? 'bg-green-50 dark:bg-green-900/20' : 'bg-slate-50 dark:bg-[var(--bg-card)]'}`}>
                  <p className={`text-sm font-semibold ${selectedMedal.unlocked ? 'text-green-600' : 'text-slate-600 dark:text-[var(--text-secondary)]'}`}>
