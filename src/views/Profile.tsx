...
export default function Profile({ onBack, onLogout, onViewParentControl }: ProfileProps) {
  const { user, currentChild, setCurrentChild, logout, refreshUser } = useAuth();
  ...
  const handleAddChild = async () => {
    ...
    try {
      ...
      setAddForm({ name: '', age: '', gender: 'male' });
    } catch (err) {
  ...
  const genderIcon = (gender?: string) => {
    if (gender === 'female') return 'face_3';
    return 'face';
  };
    ...
    >
        ...
        <div className="flex w-full flex-col gap-4">
          <div className="flex gap-4 items-center">
            ...
            <div className="flex flex-col justify-center">
              <p className="text-slate-900 text-[20px] font-bold leading-tight">
                {user?.username || '家长'}
              </p>
              <p className="text-primary font-medium text-sm">管理您的家庭与账户信息</p>
              {user?.phone && (
                <p className="text-slate-400 text-xs mt-0.5">{user.phone}</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <span className="material-symbols-outlined text-sm">
                {showAddForm ? 'remove_circle' : 'add_circle'}
              </span>
              {showAddForm ? '取消' : '添加'}
            </button>
          </div>
            ...
            {showAddForm && (
              ...
              >
                <div className="bg-primary/5 rounded-xl p-4 mb-4 space-y-3 border border-primary/10">
                  ...
                  <input
                    className="form-input w-full rounded-xl border-slate-200 bg-white text-slate-900 h-11 text-sm placeholder:text-slate-400 focus:border-primary focus:ring-1 focus:ring-primary px-3"
                    placeholder="孩子姓名 *"
                    type="text"
                    value={addForm.name}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex gap-1 bg-white rounded-xl border border-slate-200 p-1">
                      <button
                        className={`flex-1 rounded-lg text-xs font-medium py-2 transition-all ${addForm.gender === 'male' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}
                        onClick={() => setAddForm(prev => ({ ...prev, gender: 'male' }))}
                      >
                        男孩
                      </button>
                      <button
                        className={`flex-1 rounded-lg text-xs font-medium py-2 transition-all ${addForm.gender === 'female' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}
                        onClick={() => setAddForm(prev => ({ ...prev, gender: 'female' }))}
                      >
                        女孩
                      </button>
                    </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      className="flex-1 py-2.5 bg-primary text-white text-sm font-bold rounded-xl disabled:opacity-60 transition-all active:scale-[0.98]"
                ...
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg transition-all cursor-pointer ${
                    currentChild?.id === child.id
                      ...
                      : 'bg-background-light hover:bg-primary/5'
                  }`}
                  onClick={() => handleSwitchChild(child)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={e => e.key === 'Enter' && handleSwitchChild(child)}
                  aria-label={`切换到${child.name}`}
                >
                  <div className={`size-10 rounded-full flex items-center justify-center ${
                    currentChild?.id === child.id ? 'bg-primary/30' : 'bg-primary/20'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {currentChild?.id === child.id && (
                        <span className="text-[10px] bg-primary text-white px-1.5 py-0.5 rounded-full font-bold">当前</span>
                      )}
                    </div>
      ...
      <div className="p-6">
        <button
          ...
          className="w-full mt-4 text-red-400 text-sm font-medium py-3 border border-red-100 rounded-xl hover:bg-red-50 transition-colors disabled:opacity-50"
          aria-label="退出登录"
        >
