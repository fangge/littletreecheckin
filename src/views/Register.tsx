...
export default function Register() {
  ...
  const { register, addChildren } = useAuth();
  ...
  const handleStep2Submit = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '添加孩子信息失败，请重试');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex items-center px-4 pt-6 pb-2 justify-between sticky top-0 bg-white/80 dark:bg-[var(--bg-primary)]/80 backdrop-blur-md z-10 transition-colors">
        {step === 1 ? (
          ...
          <>
            ...
            <h2 className="text-slate-900 text-lg font-bold leading-tight tracking-tight flex-1 text-center">添加孩子</h2>
            <div className="size-10" />
          </>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          ...
          >
            ...
            {/* Step 1 Form: only account fields */}
        ...
        {step === 2 && (
          ...
          >
            {/* Step 2 Header */}
            <div className="px-6 pt-8 pb-4">
              <h3 className="text-slate-900 text-3xl font-bold leading-tight mb-2">添加孩子信息</h3>
              <p className="text-slate-500 text-sm">帮助孩子建立专属的成长小森林，稍后也可以再添加。</p>
            </div>
            ...
            <div className="px-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-slate-900 text-xl font-bold tracking-tight">孩子信息</h2>
                <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full font-medium">可选</span>
              </div>
              ...
              <button
                ...
                aria-label="添加另一个孩子"
              >
                <Icon name="add_circle" className="text-xl group-hover:scale-110 transition-transform" />
                <span className="font-semibold text-sm">添加另一个孩子</span>
              </button>
            </div>

            {/* Step 2 Actions */}
            <div className="mt-8 p-6 space-y-3">
              <button
              ...
              <button
                onClick={handleSkip}
                disabled={isLoading}
                className="w-full bg-slate-100 text-slate-600 py-3 rounded-xl font-medium text-sm hover:bg-slate-200 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                aria-label="跳过，稍后添加"
              >
                跳过，稍后添加
              </button>
