-- ============================================================
-- 修复 approve_task_rpc 函数的返回类型问题
-- 将 goal_title 从 TEXT 改为 VARCHAR(100) 以匹配 goals.title 的实际类型
-- ============================================================
...
RETURNS TABLE (
  task_id UUID,
  goal_title VARCHAR(100),  -- 改为 VARCHAR(100) 以匹配 goals.title 的类型
...
BEGIN
  ...
  RETURN QUERY SELECT
    p_task_id,
    v_goal.title::VARCHAR(100),  -- 显式转换为 VARCHAR(100)
    v_total_fruits,
    v_tree_completed,
    NULL::TEXT;
END;
