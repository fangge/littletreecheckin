...
奖品商店支持一次兑换多个同款奖品，并按实际数量扣除或返还果实。家长可为每个奖品配置单个孩子的总兑换上限、连续兑换上限和冷静期，避免奖品被无限重复兑换。

- ✅ **新增** `supabase/migrations/20260712150400_add_reward_redemption_quantity.sql`：兑换记录新增数量字段，兼容已有记录并默认数量为 1
- ✅ **新增** `supabase/migrations/20260712152046_add_reward_redemption_limits.sql`：奖品新增总兑换上限、连续兑换上限和冷静期配置及数据库约束
- ✅ **新增** `server/src/utils/rewardRedemptionLimits.ts`：统一计算累计兑换数量、连续兑换周期、冷静期截止时间和当前可兑换数量
- ✅ **新增** `server/test/rewardRedemptionLimits.test.ts`：覆盖总上限、连续上限、冷静期结束和连续周期重置规则
- ✅ **修改** `server/src/routes/rewards.ts`：兑换接口支持数量参数，按数量扣除/返还果实，并在服务端强制校验每个孩子的兑换限制
- ✅ **修改** `src/services/api.ts`：扩展奖品限制、兑换数量和可兑换状态类型及 API 参数
- ✅ **修改** `src/views/Store.tsx`：兑换弹窗新增数量步进器，展示剩余兑换数与冷静期，并限制本次最大兑换数量
- ✅ **修改** `src/views/RewardsManagement.tsx`：奖品表单新增三项兑换规则配置，奖品列表展示已配置的限制
- ✅ **修改** `src/views/RedemptionHistory.tsx`：兑换历史按实际数量计算果实消耗并展示多件数量
- ✅ **修改** `supabase/migrations/init.sql`：同步奖品限制和兑换数量字段，保持新数据库初始化结构一致

**功能特性**：
- 同一个奖品可在一次操作中兑换多个，自动计算总消耗和剩余果实
- 每个孩子独立累计每个奖品的兑换数量，待发放和已发放记录都会计入限制
- 支持设置奖品总兑换上限，达到上限后不可继续兑换
- 支持设置连续兑换上限与冷静天数，冷静期结束后自动开始新周期
- 商店实时显示总剩余数量、本轮剩余数量或冷静期截止时间
- 家长撤回待发放兑换时，按兑换数量完整返还果实并释放对应额度

**数据库迁移**：依次执行 `supabase/migrations/20260712150400_add_reward_redemption_quantity.sql` 和 `supabase/migrations/20260712152046_add_reward_redemption_limits.sql`

---

### v4.3 — 果实兑换人民币与兑换比例配置

新增果实兑换人民币能力，孩子可在商店提交现金兑换申请，家长可在奖品与兑换管理中配置兑换比例并确认发放。本次更新支持同时配置“多少果实 = 多少人民币”，兑换记录会保存当次比例，后续调整不会影响历史记录。

- ✅ **新增** `supabase/migrations/015_add_cash_redemptions.sql`：新增现金兑换配置表、现金兑换记录表和 `redeem_cash_rpc` 原子兑换函数
- ✅ **新增** `server/src/routes/rewards.ts`：现金兑换配置、提交兑换、确认发放、撤回兑换等后端接口
- ✅ **新增** `src/services/api.ts`：现金兑换设置、现金兑换提交、现金兑换确认/撤回 API 方法与类型定义
- ✅ **新增** `src/views/Store.tsx`：商店页新增“果实换人民币”入口，支持输入兑换果实数并预估到账金额
- ✅ **修改** `src/views/RewardsManagement.tsx`：奖品与兑换管理新增“兑换比例”页签，可配置果实数和人民币金额，并统一管理现金/奖品兑换记录
- ✅ **修改** `src/views/RedemptionHistory.tsx`：兑换历史支持展示现金兑换记录、当次兑换比例和到账金额
- ✅ **修改** `supabase/migrations/init.sql`：整合现金兑换相关表结构与 RPC，保持新库初始化脚本同步
- ✅ **修复** `supabase/migrations/015_add_cash_redemptions.sql`：兼容重复执行旧版迁移，先补列并 drop/recreate 返回结构变化的 RPC

**功能特性**：
- 孩子可用果实提交人民币兑换申请，提交后进入待发放状态
- 家长可配置任意兑换比例，例如 `50 🍎 = ¥5.00`
- 商店页实时按当前比例预估到账金额
- 家长兑换记录统一展示奖品兑换与现金兑换，可确认发放或撤回返还果实
- 兑换历史保存当次果实数、人民币金额和兑换比例，后续改比例不影响旧记录
- 数据库兑换 RPC 原子扣减果实并创建记录，避免余额和记录不一致

**数据库迁移**：执行 `supabase/migrations/015_add_cash_redemptions.sql`
